    ...
    try {
      ...
      if (Data.dataAvancement.pourcent >= 1) {
        let DataResponsable = await UtilisateursSchema.findOne({
          email: Data.responsable,
        });
        if (!DataResponsable) {
          return false;
        }
        if (DataResponsable.listeTacheResponsable.indexOf(Data._id) != -1) {
          DataResponsable.listeTacheResponsable.splice(
            DataResponsable.listeTacheResponsable.indexOf(Data._id),
            1
          );
          ...
          await NotificationTools.sendSystemNotification(
            ...
            "la tache " + Data.chemin + Data.titre + " vient d'etre termine!"
          );
        } else if (DataResponsable.listeProjets.indexOf(Data._id) != -1) {
          DataResponsable.listeProjets.splice(
            DataResponsable.listeTacheResponsable.indexOf(Data._id),
            1
          );
          ...
          await NotificationTools.sendSystemNotification(
            DataResponsable.email,
            "le projet " + Data.titre + " vient d'etre termine!"
          );
        }
        if (Data.collaborateur && Data.collaborateur != "") {
          let DataCollaborateur = await UtilisateursSchema.findOne({
            email: Data.collaborateur,
          });
          ...
          if (
            DataCollaborateur.listeTacheCollaborateur.indexOf(Data._id) != -1
          ) {
            DataCollaborateur.listeTacheCollaborateur.splice(
              DataCollaborateur.listeTacheCollaborateur.indexOf(Data._id),
              1
            );
    ...
    try {
      ...
      while (KeepGoing) {
        ...
        if (!DataTacheMere) {
          ...
          if (!DataTacheMere) {
            console.error(
              "[updateProjet] impossible de trouver le projet/tache mere ",
              IdMere
            );
            return false;
          }
        ...
        for (var i = 0; i < DataTacheMere.listeSousTaches.length; i++) {
          let DataSousTache = await TachesSchema.findById(
            DataTacheMere.listeSousTaches[i]
          );
          if (!DataSousTache) {
            console.error(
              "[updateProjet] impossible de trouver la soustaches pour le dataAvancement "
            );
            return false;
          }
          ...
          if (DataTacheMere.dateDebutEffect > DataSousTache.dateDebutEffect) {
            DataTacheMere.dateDebutEffect = new Date(
              DataSousTache.dateDebutEffect
            );
          }
          if (DataTacheMere.dateFinEffect < DataSousTache.dateFinEffect) {
            ...
            if (DataTacheMere.dateFinEffect > DataTacheMere.dateFinInit) {
              await NotificationTools.sendSystemNotification(
                DataTacheMere.responsable,
                "la tache " +
                  DataTacheMere.chemin +
                  DataTacheMere.titre +
                  " a du retard. date de fin effective:" +
                  DataTacheMere.dateFinEffect.getOKLMDate() +
                  ", date de fin initiale:" +
                  DataTacheMere.dateFinInit.getOKLMDate()
              );
            }
          }
          ...
          DataTacheMere.dataAvancement.chargeConsomme +=
            DataSousTache.dataAvancement.chargeConsomme;
          DataTacheMere.dataAvancement.chargeRestante +=
            DataSousTache.dataAvancement.chargeRestante;
          DataTacheMere.dataAvancement.chargeInitiale +=
            DataSousTache.dataAvancement.chargeInitiale;
          DataTacheMere.dataAvancement.chargeEffective +=
            DataSousTache.dataAvancement.chargeEffective;
        }
...
Object.defineProperty(Array.prototype, "flatyflat", {
  value: function (depth = 1) {
    return this.reduce(function (flatyflat, toFlatten) {
      return flatyflat.concat(
        Array.isArray(toFlatten) && depth > 1
          ? toFlatten.flatyflat(depth - 1)
          : toFlatten
      );
    }, []);
  },
